...
def test_non_owning_view_constructor_basic_functionality():
    ...
    '''
    buf = list(range(12))
    view = cnda.make_view_int32([3, 4], buf)
...
def test_non_owning_view_reads_correct_values_from_external_buffer():
    ...
    view = cnda.make_view_double([3, 4], buf)
...
def test_non_owning_view_shares_data_with_external_buffer():
    ...
    '''
    buf = [1, 2, 3, 4, 5, 6]
    v1, v2 = cnda.make_two_views_int32([2, 3], [6], buf)
...
def test_non_owning_view_data_points_to_external_buffer():
    ...
    '''
    buf = [10, 20, 30, 40]
    view = cnda.make_view_int32([2, 2], buf)
...
def test_non_owning_view_with_different_shapes():
    ...
    # 1D view
    buf1 = [1, 2, 3, 4, 5]
    v1 = cnda.make_view_int32([5], buf1)
    assert v1.ndim() == 1
    assert v1.size() == 5
    ...
    # 3D view
    buf3 = list(range(24))
    v3 = cnda.make_view_int32([2, 3, 4], buf3)
    assert v3.ndim() == 3
    assert v3.size() == 24
    s = v3.strides()
    assert s[0] == 12
    ...
    # 4D view
    buf4 = [0.0] * 120
    v4 = cnda.make_view_double([2, 3, 4, 5], buf4)
    assert v4.ndim() == 4
    assert v4.size() == 120
...
def test_non_owning_view_with_const_access():
    ...
    '''
    buf = [1, 2, 3, 4, 5, 6]
    view = cnda.make_view_int32([2, 3], buf)
...
def test_non_owning_view_with_zero_sized_dimension():
    ...
    '''
    buf = []
    view = cnda.make_view_int32([0, 5], buf)
    assert view.is_view() is True
    assert view.size() == 0
    assert view.ndim() == 2
    sh = view.shape()
